  ...
  create(context) {
    return {
      CallExpression(node) {
        ...
        if (comparison?.type !== AST_NODE_TYPES.BinaryExpression || comparison.operator !== "===" && comparison.operator !== "!==" || // eslint-disable-next-line no-prototype-builtins
...
const RULE_NAME$9 = "prefer-mock-promise-shorthand";
  ...
  create(context) {
    return {
      CallExpression(node) {
        ...
        const { matcher } = vitestFnCall;
        const matcherArg = getFirstMatcherArg(vitestFnCall);
        if (comparison?.type !== AST_NODE_TYPES.BinaryExpression || isComparingToString(comparison) || // eslint-disable-next-line no-prototype-builtins
        !EqualityMatcher.hasOwnProperty(getAccessorValue(matcher)) || !isBooleanLiteral(matcherArg))
          return;
        const [modifier] = vitestFnCall.modifiers;
  ...
  create(context) {
    return {
      CallExpression(node) {
        ...
        const { matcher } = vitestFnCall;
        const matcherArg = getFirstMatcherArg(vitestFnCall);
        if (!includesCall || matcherArg.type === AST_NODE_TYPES.SpreadElement || // eslint-disable-next-line no-prototype-builtins
